/*
  ...
  1. New Tables
    ...
    - `library_playlists` - User-created collections of content
    - `playlist_items` - Junction table for playlists and items
    - `library_comments` - User comments on library items
    - `library_upload_requests` - User submissions for new content
...
CREATE POLICY "Anyone can view unlocked library items"
  ...
  FOR SELECT
  TO public
  USING (NOT is_locked OR user_id = uid());
...
CREATE POLICY "Users can view playlist items for their playlists"
  ...
  USING (
    EXISTS (
      ...
      AND (library_playlists.user_id = uid() OR library_playlists.is_shared = true)
    )
  );
...
CREATE POLICY "Users can insert items into their playlists"
  ...
  WITH CHECK (
    EXISTS (
      SELECT 1 FROM library_playlists
      WHERE library_playlists.id = playlist_items.playlist_id
      AND library_playlists.user_id = uid()
    )
  );
...
CREATE POLICY "Anyone can view comments on unlocked items"
  ...
  USING (
    EXISTS (
      ...
      AND (NOT library_items.is_locked OR library_items.user_id = uid())
    )
  );
